$(function(){
	...
	var $fn_busca_produtos = (function(){
		...
		$body.stop().animate({ scrollTop: ($div_produtos_lista.offset().top)-60 }, 200);

		$.post("ajax/produtos-lista.php",{
			quantidade: quantidade, ordem:ordenacao, pagina:pag_atual, marcas:marcas, linhas:linhas
		}, function(data){
			$div_produtos_lista.html(data);
			$div_produtos_lista.removeClass("fx-loader");
			
			var urlNormal = window.location.href.split('?');
			history.pushState(false, false, urlNormal[0] + "?linhas=" + linhas + "&marcas=" + marcas + "&ordenacao=" + ordenacao + "&quantidade=" + quantidade + "&pagina=" + pag_atual);
		});
	});
	...
	var $fn_busca_marcas = (function(primeiravez = 0){
		...
		linhas_selecionadas = linhas_selecionadas.substring(0,(linhas_selecionadas.length - 1));
		
		$.post("ajax/consulta-marcas.php",{
	...
	$("body").delegate(".p-lista-navegacao-botao-d", "click", function(){
		//var pag_atual = parseInt($("#pag-atual").val());
		if(pag_atual < $(this).attr("data-tamanho")){
			$("#pag-atual").val(pag_atual+1);
	...
	// Se tiver alguma coisa em sessionStorage define os filtros ao carregar a página
